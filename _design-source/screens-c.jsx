...
// =========================================================
// DASHBOARD — stats, goals, streak, heatmap, chart
// =========================================================
function ScreenDashboard() {
  // 52 weeks × 7 days heatmap data
  const weeks = 26;
  const heat = Array.from({length: weeks}, (_, w) => Array.from({length: 7}, (_, d) => {
  ...
  const heatColor = (v) => ['var(--surface-2)','oklch(0.40 0.10 30)','oklch(0.50 0.13 30)','oklch(0.58 0.15 30)','var(--accent)'][v];

  // weekly chart data
  const weeksData = [
    [1,1240],[2,2810],[3,4520],[4,6800],[5,8200],[6,9100],[7,10840],[8,12600],[9,14920],[10,17040],[11,19280],[12,21540],
  ];
  const target = weeksData.map(([w])=>[w, w*1800]);
  const chartW = 720, chartH = 220, padL = 36, padB = 28, padT = 14, padR = 14;
  const xMax = 12, yMax = 22000;
  const xs = (x) => padL + (x-1)/(xMax-1) * (chartW - padL - padR);
  const ys = (y) => chartH - padB - (y/yMax) * (chartH - padT - padB);
  ...
  return (
    <div className="as as-app as-app--no-right" style={{height:'100%'}}>
      <aside className="sb">
        <div className="sb-head"><div className="sb-book-title">{NOVEL.title}</div><div className="sb-book-author">дэшборд книги</div></div>
        <nav style={{padding:'14px 8px',display:'flex',flexDirection:'column',gap:2}}>
          {[['book','Манускрипт',false],['layout','Дэшборд',true],['char','Персонажи',false],['map','Карта мира',false],['clock','Хронология',false]].map(([n,l,on])=>(
        ...
        <div style={{padding:'0 14px',display:'flex',flexDirection:'column',gap:4}}>
          {['7 дней','30 дней','3 месяца','Всё время'].map((l,i)=>(
            <button key={l} className={'sb-item'+(i===2?' sb-item--on':'')}><span/><span className="sb-item-title">{l}</span><span/></button>
          ))}
        </div>
      </aside>
      ...
      <main style={{display:'flex',flexDirection:'column',background:'var(--bg)',overflow:'hidden'}}>
        <div className="tb" style={{justifyContent:'space-between'}}>
          <span style={{font:'500 13px var(--font-ui)'}}>Дэшборд · Северный архив</span>
          <div style={{display:'flex',gap:8,alignItems:'center'}}>
            <span className="chip">обновлено только что</span>
            <button className="btn"><Icon name="download" size={14}/> Отчёт</button>
          </div>
        </div>
        ...
        <div style={{flex:1,minHeight:0,overflow:'auto',padding:'28px 32px 40px'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1.4fr 1fr',gap:16,marginBottom:20}}>
            ...
            <div style={{background:'var(--surface)',border:'1px solid var(--border-soft)',borderRadius:12,padding:'20px 24px'}}>
              ...
              <p style={{fontSize:12,color:'var(--ink-3)',marginBottom:18}}>При текущем темпе — <span style={{color:'var(--ink)'}}>23 декабря 2026</span></p>
            ...
            <div style={{background:'var(--surface)',border:'1px solid var(--border-soft)',borderRadius:12,padding:'20px 24px',display:'flex',flexDirection:'column'}}>
              ...
              <div style={{display:'flex',alignItems:'baseline',gap:12,marginBottom:6}}>
                ...
                <div style={{font:'400 13px var(--font-ui)',color:'var(--ink-2)'}}>дней<br/><span style={{color:'var(--ink-3)',fontSize:11}}>лучшая серия — 24 дня</span></div>
              </div>
              {/* week dots */}
          ...
          <div style={{background:'var(--surface)',border:'1px solid var(--border-soft)',borderRadius:12,padding:'20px 24px'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'baseline',marginBottom:14}}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:8,font:'400 11px var(--font-mono)',color:'var(--ink-3)'}}>
                меньше {[0,1,2,3,4].map(v=><span key={v} style={{width:11,height:11,borderRadius:2,background:heatColor(v),display:'inline-block'}}/>)} больше
              </div>
            </div>
            <div style={{display:'flex',gap:12,alignItems:'flex-start'}}>
              ...
              <div style={{flex:1,overflow:'hidden'}}>
                <div style={{display:'flex',gap:14,fontSize:10,fontFamily:'var(--font-mono)',color:'var(--ink-4)',marginBottom:6,marginLeft:0}}>
