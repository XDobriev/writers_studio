// Авторская студия — sample manuscript content + shared chrome bits
// Loaded BEFORE other screens. Exposes content + chrome helpers on window.

const NOVEL = {
  title: 'Северный архив',
  author: 'Анна Корвин',
  genre: 'Тёмное фэнтези',
  description: 'Картограф ордена Тереи получает приказ восстановить карту города, которого больше нет.',
  chapters: [
  ...
  margins: [
    { id: 1, kind: 'idea',      time: 'вчера',   text: 'Сделать упоминание матери Инея в самом начале — отзеркалит финал.', quote: 'тоньше и темнее, чем шёлк, но плотнее' },
    { id: 2, kind: 'question',  time: '2 ч',     text: 'Откуда у трактирщика серебряный ключ? Это ружьё на стене или просто деталь?', quote: 'Серебряный ключ висел на гвозде' },
    { id: 3, kind: 'todo',      time: '5 мин',   text: 'Проверить — Иней могла быть в Корне ребёнком? Нужен флэшбек в гл. 7.', quote: 'я родилась там же, где и снег' },
    { id: 4, kind: 'important', time: '10 мин',  text: 'НЕ называть имя двенадцатого картографа до главы 8.', quote: 'один из нас не вернулся' },
  ],
  versions: [
...
// Manuscript paragraphs for chapter 1 (with margin highlights, spell errors, edits)
window.SAMPLE_PROSE = `
...
// chrome — produce Sidebar / Toolbar etc as static HTML helpers (returned as React nodes via dangerouslySetInnerHTML wrappers).
window.dangerHTML = (html) => ({ __html: html });
