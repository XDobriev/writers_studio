...
**Проверить:** (1) для Outline/Notes/Timeline — проверить, ставится ли фокус на поле названия/текста сразу после quick-create; добавить `autoFocus` по аналогии, если нет. (2) Более полное решение (не реализовано — большая правка, требует отдельного решения): удалять черновик автоматически при уходе без ввода, либо не персистить запись до первого символа — осознанно не делать без обсуждения, см. `feedback_ask_before_ux_decisions`.
