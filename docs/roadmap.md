...
**✅ Пройдено (2026-07-09)** — все 12 сценариев (регистрация, вход/выход, автосохранение, мобильный split, боевой платёж Robokassa, персонажи, связи, экспорт DOCX/FB2, хронология/заметки/карта, мобильные модалки) проверены без ошибок.
...
### R9. Замысел книги (до-письменное планирование)

**Что это:** страница `/books/:id/plan` — один rich-text документ на книгу с
оглавлением по заголовкам. Слой замысла ДО письма; отличается от Timeline
(внутримировая хронология) и Outline/Corkboard (структура глав по ходу письма).

**Шесть полей вычеркнуты.** Исходная формулировка (логлайн, тема, конфликт, ставки,
синопсис, арки) написана из головы и опровергнута реальным планировочным документом:
из шести полей автор написал одно. Все известные методики (Save the Cat, снежинка,
путь героя, три акта) физически являются списком заголовков с прозой — документ есть
их объединение, набор полей выражает ровно одну. Шаблонов в v1 ноль: шаблон = контент,
добавить потом стоит одну константу, убрать — уже нельзя.

**Статус:** дизайн-спек готов — `docs/superpowers/specs/2026-07-15-book-plan-design.md`.
Спек №3 из 3. Делать после R8.

**Правка в R7 (реализованном):** принципы уровня серии должны шариться, а не
копироваться в продолжение — инвариант вида «имя рыси — Баки, держать везде» теряет
смысл, если разойдётся в две редакции. Детали в спеке.
...
странице даёт ему лишь паритет с Word.
