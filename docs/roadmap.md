...
### Админ-таблицы обрезают контент вместо скролла

**Симптом:** `.admin-table-wrap` использует `overflow: hidden` — при узком экране или широкой таблице (много колонок/длинные значения) содержимое молча обрезается, без горизонтального скролла.

**Воспроизвести:** 1. Зайти в `/admin` под аккаунтом с ролью admin → 2. Открыть вкладку с широкой таблицей (пользователи/платежи/аудит-лог) на узком окне → 3. ожидаю горизонтальный скролл или адаптивную вёрстку, вижу обрезанные колонки без возможности прокрутки.

**Файлы:** `src/styles/design-system.css` (`.admin-table-wrap`, ~строка 1856), `src/components/admin/*.tsx`.

**Контекст:** тот же класс бага, что чинили в статус-баре редактора (`.status`) 2026-07-14 — там фикс был `overflow-x: auto` + скрытый скроллбар по образцу `.tb`. Не проверено и не исправлено в эту сессию: e2e-тест-аккаунт не имеет прав admin, нужен реальный админ-логин.

**Проверить:** после фикса — `getBoundingClientRect` таблицы на узком viewport, `scrollLeft` должен реально сдвигаться.
