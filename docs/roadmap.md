...
### R7. Статус-бар на всю ширину

**Что это:** сейчас `<StatusBar>` находится внутри `<main>` (центральная колонка грида), поэтому не растягивается под сайдбар и правую панель. Нужно вынести его за пределы грида, чтобы он занимал всю ширину приложения.

**Что сделать:** в `EditorHybrid.tsx` обернуть текущий grid-контейнер во flex-column, перенести `<StatusBar>` ниже грида. Пропсы не меняются — все данные уже есть в `EditorHybrid`.

**Файлы:** `src/components/EditorHybrid.tsx`
**Проверить:** статус-бар тянется от левого края сайдбара до правого края правой панели во всех режимах (Studio, Split, Рукопись, Фокус)

---

### R8. Персонажи главы в правой панели

**Что это:** показывать список персонажей текущей главы прямо в правой панели редактора (вкладка «Заметки»), не переходя на страницу Characters. С POV-меткой на нужном персонаже.

**Что исследовать:**
- `chapter_characters` (синхронизируются через `syncBacklinks` в `Editor.tsx` при автосохранении) — данные уже есть в БД
- Запрос: `useQuery(QUERY_KEYS.chapterCharacters(chapterId))` — проверить, есть ли такой ключ в `queries.ts`
- POV-запись: `chapter_pov_characters` или поле в `chapters` — нужно найти где хранится
- Нужно ли lazy-загружать аватары персонажей или показывать только инициалы
- UX: отдельная секция «Персонажи» под заметками в том же табе, или новый таб

**Файлы:** `src/components/RightPanel.tsx`, `src/lib/queries.ts`, `src/lib/crossrefs.ts`
**Триггер:** после ресёрча структуры данных — оценить объём и решить, делать или нет.
