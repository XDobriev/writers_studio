...
2. ✅ Создать магазин → подать заявку на активацию (2026-06-09, ожидание одобрения)
3. После одобрения: ЛК → Технические настройки → сгенерировать **Пароль #1** и **Пароль #2** → указать Result/Success/Fail URL
4. Подключить РобоЧеки СМЗ: ЛК → Фискализация → «Самозанятые ФНС» → «Отправить заявку» → подтвердить в приложении «Мой налог»
