...
_Обновлён: 2026-06-07 (юридические документы обновлены; два чекбокса согласия в Auth; §2a локализация данных добавлена)_
...
### 2a. Перенос Supabase на VPS — локализация данных (ФЗ-242) ⚠️
...
### 3a. GEO — внешние упоминания (постоянно после запуска)
...
**Deps:** §4 (Robokassa, рекуррентные платежи)
...
### 9a. Флаг «регистрация закрыта»

**Что это:** механизм временного отключения регистрации без редеплоя — например, при миграции на self-hosted Supabase или технических работах.

**Как реализовать:**
1. В таблице `app_settings` добавить строку `key = 'registration_open'`, `value = 'true'`
2. Читать флаг при старте приложения (или на странице регистрации) через `supabase.from('app_settings').select('value').eq('key', 'registration_open')`
3. Если `value = 'false'` — показывать заглушку «Регистрация временно закрыта» вместо формы
4. Дополнительно: в Supabase Dashboard → Auth → Settings → отключить «Enable email signups» (мгновенно блокирует на уровне backend)

**Переключение:** Supabase Table Editor → `app_settings` → изменить `value`. Без коммита и деплоя.

**Файлы:** `src/pages/Landing.tsx` или компонент регистрации, `src/lib/queries.ts` (хук `useAppSettings`)
**Проверить:** `registration_open = false` → форма регистрации заменяется заглушкой; `registration_open = true` → работает штатно
...
### §R6. AI-генерация синопсиса главы
...
### §R7. Позиционирование обложки (crop/pan)
