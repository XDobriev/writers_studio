...
### Export.tsx: футер сжимает имя файла и размер до нечитаемого состояния на узком мобильном (320px)

**Симптом:** нижняя панель страницы экспорта (`имя файла` + `~N КБ` + кнопки «Отмена»/«Скачать») — единый flex-ряд без переноса. На 320px кнопки «Отмена» и «Скачать EPUB» съедают почти всю ширину, а левый блок с именем файла и оценкой размера сжимается до нескольких символов («о…», «~18» и «КБ» на трёх отдельных строках).
**Контекст:** найдено 2026-08-09 (`/loop`) при проверке `Export.tsx` на 320px viewport вместе со схожим багом переноса подписи «текстовые форматы» (та часть уже исправлена в этой же итерации — строка 304). Футер сложнее: `<div style={{minWidth:0, overflow:'hidden'}}>` с именем файла корректно получает `text-overflow:ellipsis`, но у соседних кнопок нет `flex-shrink:0`/адаптива, поэтому именно текстовый блок теряет почти всю ширину первым.
**Файлы:** `src/pages/Export.tsx:508-529` (Footer).
**Проверить:** на 320px ширине разложить футер в две строки на мобильном (имя файла + размер сверху, кнопки снизу на всю ширину) — например через `flex-wrap: wrap` с `flex-basis: 100%` у текстового блока ниже брейкпоинта, по аналогии с уже существующим `.export-fmt-grid` `@media (max-width: 560px)` в `design-system.css:1615`.
