...
### §PERF-1. Виртуализация Timeline

**Когда делать:** при ощутимом тормозе при 100+ событиях в одной книге.

**Что это:** Timeline рендерит горизонтальные лейны с DnD (`TimelineLane.tsx`). При росте числа событий все DOM-узлы живут в памяти. Нужна горизонтальная виртуализация — сложнее Notes (там вертикальный скролл через `useVirtualizer`).

**Файлы:** `src/components/TimelineLane.tsx`, `src/pages/Timeline.tsx`.
