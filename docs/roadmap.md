...
### R9. План книги (до-письменное планирование)

**Что это:** book-level страница замысла — логлайн, тема/премиса, центральный
конфликт, ставки, синопсис книги, арки героев. Сейчас этому месту нет (только
размазать по Notes). Отличается от Timeline (внутримировая хронология) и
Outline/Corkboard (структура глав по ходу письма) — это слой замысла ДО письма.
Опционально второй фазой — акт/beat-структура поверх Corkboard.

**Статус:** Спек №3 из 3. Делать после R8.

---

### R10. Кнопка «Создать продолжение» в меню карточки книги

**Что это:** второй вход в перенос данных в продолжение. Сейчас единственный вход —
галка «Это продолжение серии» в модале «Новая книга»: она невидима, пока не нажмёшь
«Новая книга», а человек, дописавший книгу 1, смотрит на книгу 1.

**Что сделать:** пункт в меню `BookCard` → открывает тот же модал «Новая книга» с уже
взведённой галкой и предзаполненным `seriesTransfer.sourceBookId`. Один flow, два входа —
дублируется только предзаполнение state, не код переноса.

**Решение принято** 2026-07-15 (обсуждались также Dashboard книги и отказ от второго входа).
