...
### Close-кнопки без `.icon-close-btn` вне карты — тач-таргет не проверен

**Контекст:** найдено при фиксе StampPopup-попапа на карте (2026-08-06, `/loop`) — тот же паттерн (инлайн `×`-кнопка без `.icon-close-btn`) встречается ещё в `Notes.tsx:638` (модал заметки, инлайн `width:28,height:28` поверх `tb-btn`), `SettingsModal.tsx:83`, `UpgradeModal.tsx:106`, `VersionModal.tsx:241`. Не проверено вживую на мобилке в этом проходе — карта была приоритетнее (реальный dead-end, а не только размер таргета).
**Проверить:** для каждого файла — открыть на мобильном вьюпорте (`hasTouch:true, isMobile:true`), измерить `getBoundingClientRect()` кнопки закрытия; если < 44×44, перевести на `.icon-close-btn` по образцу `WorldMap.tsx`/`StampPopup.tsx`.
