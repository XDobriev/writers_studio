...
_Обновлён: 2026-06-10_ — флэш лендинга при refresh закрыт по корню: 5-сек fallback в auth.tsx открывал роуты в разлогиненное состояние во время медленного рефреша истёкшего токена (VPS /sb прокси). Таймаут сделан условным (15 с при наличии токена)
...
### Vercel-деплой раздаёт устаревший бандл
**Симптом:** `avtorskaya-studiya.vercel.app` показывает старую версию приложения — нет фокус-режима, нет шрифт-редактора, только 5 базовых аудио. Редирект на avtorstudio.com не работает.
**Причина:** `vercel.json` был изменён в `8076c31` (добавлен redirect на avtorstudio.com + playwright в buildCommand). Но следующие 2 коммита (`ac5d3f9`, `77e4b0e`) чинили падения Vercel-билда из-за Playwright. Пока билды падали, redirect так и не задеплоился — Vercel застрял на последнем успешном деплое (до `8076c31`).
**Решение (выбрать):**
- **A. Убедиться, что текущий redirect задеплоен** — триггернуть новый деплой на Vercel вручную (`vercel deploy --prod`), проверить что `/(.*) → avtorstudio.com` работает.
- **B. Сделать Vercel реальным резервным деплоем** — убрать redirect, вернуть SPA-rewrite + X-Robots-Tag noindex. Потребует настройки `VITE_SUPABASE_URL` и `VITE_SUPABASE_ANON_KEY` в Vercel env vars.
**Проверить:** `curl -I https://avtorskaya-studiya.vercel.app/books` должен вернуть 301 на avtorstudio.com (вариант A) или 200 с актуальным HTML (вариант B).
...
**Что это:** отдельная страница `/profile` с возможностью загрузить аватар и задать отображаемое имя. Аватар показывается в Sidebar (сейчас — только инициалы), в SidebarFoot и потенциально на странице Characters (POV-автор). (нет, пов-автор не предусмотрено)
Как идея, прогружать автоматом изображение, если оно уже есть у гугл аккаунта или у телеграмма. Если нет, то должна быть возможность загруить свое фото, если пользователь хочет.
