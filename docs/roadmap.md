...
- **L11 — фронтенд ещё на email (= L7).** [AccountMenu.tsx:14](../../src/components/AccountMenu.tsx#L14) показывает ссылку «Админ» по `user.email === ADMIN_EMAIL`. Это только UI-гейт (реальная защита — серверный `is_admin()`), но для полноты стоит перевести и его на `user.app_metadata.role === 'admin'`. Низкий приоритет.
