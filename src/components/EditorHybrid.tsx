...
function PageHeader({ mode, setMode }: ModeSegmentProps) {
  return (
...
function GoalArc() {
  const r = 18;
  const c = 2 * Math.PI * r;
  const ratio = 348 / 1000;
  return (
    <div style={{
      position: 'absolute', right: 24, bottom: 24,
      display: 'flex', alignItems: 'center', gap: 12,
      background: 'var(--bg-deep)', border: '1px solid var(--border)', borderRadius: 14,
      padding: '10px 14px 10px 12px', boxShadow: '0 8px 28px rgba(0,0,0,.25)', zIndex: 5,
    }}>
...
export function EditorHybrid({ defaultMode = 'studio' }: EditorHybridProps) {
  ...
  return (
    <div className="as" style={{ height: '100%', display: 'grid', gridTemplateColumns: cols, background: 'var(--bg)', position: 'relative', transition: 'grid-template-columns 220ms cubic-bezier(.2,.7,.3,1)' }}>
      ...
      <main style={{ display: 'flex', flexDirection: 'column', minWidth: 0, background: 'var(--bg)', position: 'relative', overflow: 'hidden' }}>
        {isPage ? <PageHeader mode={mode} setMode={setMode} /> : <StudioToolbar mode={mode} setMode={setMode} />}

        <div className="sheet-wrap" style={{ padding: isPage ? '48px 56px 110px' : '36px 32px 0' }}>
          <div
            className="sheet"
            style={{ width: sheetWidth, padding: sheetPad }}
