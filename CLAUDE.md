...
Дальше:
- **Заход 10+** — focus/split на реальных данных, writing-snapshots для графиков дэшборда, графическая карта по координатам, горизонтальная timeline.

Демо-экраны (focus, split) пока рендерятся на демо-данных из `src/data/sample.ts`. Все основные сущности (главы, персонажи, связи, хронология, локации, экспорт, дэшборд) переехали на реальные данные.
...
**Focus / Split** — демо, не в проверке.
