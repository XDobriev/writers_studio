...
- `src/lib/useOutlineMutations.ts` — `onCreate`, `onDelete`, `onRename`, `onDragEnd`, `onPovChanged`, `onRenumber`: mutation-слой глав для `Outline.tsx`; `onRenumber` — bulk upsert через `renumberChapters` только для глав с паттерном «Глава N»; использует хелперы `chapterMutations.ts` внутри.
...
- [docs/features/payments.md](docs/features/payments.md) — оплата (Робокасса), РобоЧеки СМЗ, фискализация, Secrets.
